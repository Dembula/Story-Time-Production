      ...
      <LandingReveal className="mx-auto max-w-4xl text-center">
        ...
        <div className="mt-8 grid grid-cols-2 gap-3 sm:mt-10 sm:grid-cols-4 sm:gap-4">
          {items.map((item) => (
            <div
              key={item}
              className="storytime-panel rounded-2xl border border-orange-400/10 px-3 py-3.5 text-xs font-medium text-slate-200 transition hover:-translate-y-0.5 hover:border-orange-300/25 sm:px-4 sm:py-4 sm:text-sm"
            >
              {item}
            </div>
          ))}
