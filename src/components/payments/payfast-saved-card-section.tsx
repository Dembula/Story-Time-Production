 ...
 * Starts an R0 tokenization (subscription_type=2) so Story Time can charge renewals later.
 */
export function PayFastSavedCardSection({
  returnPath,
  title = "Billing card (PayFast)",
  description = "Save a card once through PayFast to pay Story Time packages and renewals. This is not your KYC/KYB payout banking — wallet withdrawals still use the bank details from verification.",
  className = "",
}: {
  returnPath?: string;
  title?: string;
  ...
  return (
    ...
    >
      <CheckoutModal
        ...
        subtitle="You will authorize R0.00 — nothing is charged today. Your bank may ask for 3D Secure. If that step fails, try another card or contact your bank, then try again."
      ...
      <div className="mt-4 flex items-start gap-2 rounded-xl border border-sky-400/20 bg-sky-500/10 px-3 py-2.5 text-xs text-sky-100/90">
        ...
        <p>
          PayFast shows <span className="font-medium text-sky-50">ZAR R 0.00</span> for card authorization.
          Complete 3D Secure when your bank asks — that confirms the card without charging you. This never
          replaces KYC/KYB bank documents used for payouts.
        </p>
      </div>
