...
/** A call sheet is stale when its shoot day changed after the sheet was generated. */
function isCallSheetStale(sheet: { createdAt: Date }, shootDay: { updatedAt: Date } | null): boolean {
  if (!shootDay) return false;
  return sheet.createdAt.getTime() < shootDay.updatedAt.getTime();
}
...
export async function GET(
  ...
  if (sheetId) {
    ...
    return NextResponse.json({
      callSheet: {
        ...sheet,
        stale: isCallSheetStale(sheet, sheet.shootDay),
        parsed: {
