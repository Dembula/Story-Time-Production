...
export async function GET(req: NextRequest) {
  ...
  if (user.role !== "ADMIN" && !requiresPayoutKyc(user.role)) {
  ...
  const verification = await prisma.payoutKycVerification.findUnique({
    where: { id: verificationId },
    include: { payoutKycProfile: { select: { userId: true } } },
  });
  if (!verification) return NextResponse.json({ error: "Document not found." }, { status: 404 });

  if (user.role !== "ADMIN" && verification.payoutKycProfile.userId !== user.id) {
    return NextResponse.json({ error: "Forbidden" }, { status: 403 });
  }
