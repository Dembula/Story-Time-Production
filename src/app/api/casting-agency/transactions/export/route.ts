...
export async function GET() {
  ...
  const rows = await listPayeeCompletedTransactionsForExport(user.id, MARKETPLACE_TRANSACTION_TYPE.CAST_INQUIRY);
  const csv = completedMarketplaceTransactionsCsv(rows);
  return new NextResponse(csv, {
    status: 200,
    headers: {
      "Content-Type": "text/csv; charset=utf-8",
      "Content-Disposition": 'attachment; filename="casting-transactions.csv"',
    },
  });
}
