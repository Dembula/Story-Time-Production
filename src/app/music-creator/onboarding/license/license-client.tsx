    ...
    <div className="space-y-6">
      <div className="rounded-2xl border border-slate-700 bg-slate-800/30 p-4 mb-6">
        ...
        <p className="text-slate-400 text-sm mt-1">
          Your music will be available on Story Time for sync licensing and the music library. You keep ownership; the license grants us the right to stream and offer your tracks to filmmakers. Choose yearly (unlimited uploads) or pay per track.
        </p>
      </div>
      <button type="button" onClick={() => setType("YEARLY_R89")} className={`w-full p-6 rounded-2xl border text-left transition ${type === "YEARLY_R89" ? "border-orange-500 bg-orange-500/10" : "border-slate-700 bg-slate-800/30"}`}>
        <div className="flex justify-between items-center">
          <div>
            <h3 className="text-lg font-semibold text-white">Yearly license</h3>
            <p className="text-slate-400 text-sm mt-1">Unlimited track uploads for 1 year. Distribute as many tracks as you want.</p>
            <p className="text-2xl font-bold text-orange-500 mt-2">R89</p>
          </div>
          {type === "YEARLY_R89" && <Check className="w-6 h-6 text-orange-500" />}
        </div>
      </button>
      <button type="button" onClick={() => setType("PER_UPLOAD_R10")} className={`w-full p-6 rounded-2xl border text-left transition ${type === "PER_UPLOAD_R10" ? "border-orange-500 bg-orange-500/10" : "border-slate-700 bg-slate-800/30"}`}>
        <div className="flex justify-between items-center">
          <div>
            <h3 className="text-lg font-semibold text-white">Pay per upload</h3>
            <p className="text-slate-400 text-sm mt-1">R10 per track. Best if you only plan to publish a few tracks.</p>
