...
export default function AdminPayoutVerificationPage() {
  ...
  async function openPrivateDocument(verificationId: string) {
    const res = await fetch(`/api/payout-kyc/documents/signed-url?verificationId=${encodeURIComponent(verificationId)}`);
  ...
  return (
    <div className="space-y-4 text-slate-100">
      ...
      <p className="text-sm text-slate-400">Approve or reject creator and marketplace accounts before payouts are released.</p>
        ...
        {(data?.profiles ?? []).map((profile: any) => (
          ...
          >
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                <p className="text-sm font-semibold text-white">{profile.legalName || profile.user?.name || "Unnamed"}</p>
                ...
                <p className="text-xs text-slate-500">{profile.accountRole} · {profile.entityType}</p>
        ...
        {selectedProfile ? (
          <div className="rounded-xl border border-slate-800 bg-slate-950/60 p-4">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div>
                <p className="text-lg font-semibold text-white">{selectedProfile.legalName || selectedProfile.user?.name || "Unnamed"}</p>
                <p className="text-xs text-slate-400">{selectedProfile.user?.email}</p>
              </div>
              <div className="flex gap-2">
                <button className="rounded border border-slate-700 px-2 py-1 text-xs" onClick={() => review.mutate({ payoutKycProfileId: selectedProfile.id, status: "UNDER_REVIEW" })}>Under review</button>
                <button className="rounded border border-emerald-900 px-2 py-1 text-xs text-emerald-300" onClick={() => review.mutate({ payoutKycProfileId: selectedProfile.id, status: "APPROVED", note: noteByProfile[selectedProfile.id] })}>Approve payouts</button>
                <button className="rounded border border-red-900 px-2 py-1 text-xs text-red-300" onClick={() => review.mutate({ payoutKycProfileId: selectedProfile.id, status: "REJECTED", note: noteByProfile[selectedProfile.id] || "Documentation incomplete or could not be verified." })}>Reject</button>
              </div>
            </div>
