...
export default function FunderDealsPage() {
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-8 text-slate-100">
      ...
      <p className="mt-2 text-sm text-slate-400">Run deals end-to-end: negotiation, term sheets, contracts, signatures, and payment milestones.</p>
      <div className="mt-6 space-y-3">
        {deals.map((deal: any) => (
          <div key={deal.id} className="rounded-lg border border-slate-800 bg-slate-950/60 p-4">
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              <div className="flex gap-2">
                <button className="rounded border border-slate-700 px-2 py-1 text-xs" onClick={() => mutate.mutate({ action: "DECIDE_DEAL", dealId: deal.id, status: "FUNDED" })}>
                  Accept & fund
                </button>
                <button className="rounded border border-red-900 px-2 py-1 text-xs text-red-300" onClick={() => mutate.mutate({ action: "DECIDE_DEAL", dealId: deal.id, status: "REJECTED", rejectionReason: "Risk profile mismatch." })}>
                  Reject
                </button>
                <button className="rounded border border-slate-700 px-2 py-1 text-xs" onClick={() => mutate.mutate({ action: "GENERATE_CONTRACT", dealId: deal.id })}>
                  Generate contract
                </button>
                <button className="rounded border border-slate-700 px-2 py-1 text-xs" onClick={() => mutate.mutate({ action: "SIGN_CONTRACT", dealId: deal.id })}>
                  Sign
                </button>
              </div>
            </div>
