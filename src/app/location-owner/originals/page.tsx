...
interface Membership {
  ...
  project: {
    ...
    members: { id: string; role: string; department: string | null; status: string; user: { id: string; name: string | null; role: string } }[];
...
export default function LocationOriginalsPage() {
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto space-y-8">
      <div>
        ...
        <p className="text-slate-400">Supply locations for Story Time Original productions. See your assigned projects and manage location commitments.</p>
      </div>

      <div className="grid grid-cols-2 gap-4">
        <div className="bg-slate-800/50 border border-slate-700/50 rounded-xl p-4">
          <p className="text-xs text-slate-400">Active Productions</p>
          <p className="text-2xl font-bold text-white">{active.length}</p>
        </div>
        <div className="bg-yellow-500/5 border border-yellow-500/20 rounded-xl p-4">
          <p className="text-xs text-yellow-400">Pending Invitations</p>
      ...
      {invites.length > 0 && (
        <div className="bg-yellow-500/5 border border-yellow-500/20 rounded-xl p-5 space-y-3">
          <p className="text-white font-medium flex items-center gap-2">
            <Star className="w-5 h-5 text-yellow-400" /> Story Time wants your location for {invites.length} production{invites.length > 1 ? "s" : ""}
          </p>
          {invites.map((m) => (
      ...
      <div className="space-y-4">
            ...
            {expanded === m.id && (
              <div className="border-t border-slate-700/50 p-5 bg-slate-900/30 space-y-3">
                ...
                <div className="flex flex-wrap gap-2">
                  {m.project.members.filter((mem) => mem.status === "ACTIVE").map((mem) => (
                    <span
                      key={mem.id}
                      className={`text-xs px-2.5 py-1 rounded-lg border ${mem.department === "LOCATION" ? "border-blue-500/30 bg-blue-500/5 text-blue-400" : "border-slate-700/50 text-slate-300"}`}
                    >
        ...
        {active.length === 0 && (
          <div className="bg-slate-800/30 border border-slate-700/50 rounded-xl p-10 text-center">
            ...
            <p className="text-slate-400">No active Originals productions yet. Invitations from Story Time will appear here when your location is needed.</p>
          </div>
        )}
      </div>
    </div>
