...
import { MapPin, MessageCircle, Clock, CheckCircle, XCircle, TrendingUp, DollarSign } from "lucide-react";
...
export default function LocationOwnerDashboard() {
  ...
  const totalMessages = bookings.reduce((acc, r) => acc + r._count.messages, 0);
  const revenue = bookings
    .filter((b) => b.status === "APPROVED" && b.location.dailyRate && b.startDate && b.endDate)
    .reduce((sum, b) => {
      const start = new Date(b.startDate!).getTime();
      const end = new Date(b.endDate!).getTime();
      const days = Math.max(1, Math.ceil((end - start) / (24 * 60 * 60 * 1000)));
      return sum + (b.location.dailyRate! * days);
    }, 0);
  ...
  return (
    <main className="max-w-7xl mx-auto px-6 md:px-12 py-10 space-y-10">
      <div>
        ...
        <p className="text-slate-400 text-sm">Manage your properties and shoot bookings</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        {[
          ...
          { label: "Revenue (est.)", value: `$${revenue.toLocaleString()}`, icon: DollarSign, color: "text-emerald-400" },
        ].map((s) => (
