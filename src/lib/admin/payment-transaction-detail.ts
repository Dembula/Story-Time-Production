...
import {
  ...
  paymentPurposeLabel,
  type AdminMarketplaceTransactionDetail,
  type AdminPaymentRecordDetail,
  type RevenueRouteLine,
} from "@/lib/admin/payment-transaction-detail.types";

export type { AdminMarketplaceTransactionDetail, AdminPaymentRecordDetail, RevenueRouteLine };
  ...
  return {
    ...
    lines: [
      ...payfastFeeLines,
      {
        ...
        accountType: "PLATFORM_REVENUE",
        amount: settlementAmount,
        description: paymentPurposeLabel(purpose),
      },
    ],
  };
}
...
export async function getAdminPaymentRecordDetail(paymentRecordId: string): Promise<AdminPaymentRecordDetail | null> {
  ...
  return {
    ...
    purposeLabel: paymentPurposeLabel(payment.purpose ?? ""),
    amount: Number(payment.amount),
