...
export function adminRevenueBundleToCsv(bundle: AdminRevenueBundle): string {
  ...
  const summary = rowsToCsv(
    ...
    [
      ...
      ["appleIapRevenue", bundle.appleIap?.revenue ?? 0],
      ["appleIapCount", bundle.appleIap?.count ?? 0],
      ["appleIapSubscriptions", bundle.appleIap?.subscriptionCount ?? 0],
      ["appleIapPpv", bundle.appleIap?.ppvCount ?? 0],
