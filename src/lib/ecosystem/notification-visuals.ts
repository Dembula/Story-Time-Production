...
export function notificationTypeIcon(type: string): LucideIcon {
  ...
  if (t.includes("UPLOAD") || t.includes("STREAM_READY") || t.includes("PUBLISH") || t.includes("CONTENT") || t.includes("CATALOGUE")) return Film;
...
export function notificationTypeAccent(type: string): string {
  ...
  if (t.includes("UPLOAD_FAILED") || t.includes("MODER") || t.includes("REJECT")) return "text-amber-300";
  if (t.includes("UPLOAD_COMPLETE") || t.includes("STREAM_READY")) return "text-emerald-300";
